#!/usr/bin/env node
/**
 * scripts/import-germany-csv.js
 *
 * Reads data/maps/germany.csv and regenerates:
 *   1. data/videos/germany.ts  (video catalog for pages + search)
 *   2. data/maps/germany.ts    (Mapbox map features)
 *
 * Usage:
 *   node scripts/import-germany-csv.js
 *   node scripts/import-germany-csv.js --dry-run
 */
'use strict';
...
const CSV_PATH      = path.join(ROOT, 'data', 'maps', 'germany.csv');
const CATALOG_PATH  = path.join(ROOT, 'data', 'videos', 'germany.ts');
const MAP_PATH      = path.join(ROOT, 'data', 'maps', 'germany.ts');
...
const col = {}; headerRow.forEach((h, i) => { col[h] = i; });
...
const THEME_KEYWORD_MAP = [
  { keywords: ['historic center', 'historic centres', 'old town'], tag: 'historic-center' },
  { keywords: ['old town', 'medieval'], tag: 'old-town' },
...
console.log(`\n${'═'.repeat(60)}`);
console.log(`  import-germany-csv${dryRun ? '  [DRY RUN]' : ''}`);
console.log(`${'═'.repeat(60)}`);
console.log(`  CSV rows:     ${dataRows.length}`);
console.log(`  Total tours:  ${allTours.length}`);
console.log(`  Ready tours:  ${readyTours.length}\n`);

// ─── 1. Generate data/videos/germany.ts ──────────────────────────────────────

function jsonStr(val) { return JSON.stringify(val, null, 2).replace(/\n/g, '\n  '); }
...
console.log(`  [1/2] data/videos/germany.ts — ${readyTours.length} ready tours`);

// ─── 2. Generate data/maps/germany.ts ────────────────────────────────────────
...
function toMapFeature(t) {
  ...
  return {
    tourId: t.tourId, country: t.country, region: t.region,
    city: t.city, location: t.location, title: t.title, slug: t.slug,
    videoType: t.videoType, themes: toMapThemeTags(t.themes),
    filmedDateIso: t.filmedDateIso || null, filmedYear: t.filmedYear,
    youtubeUrl: t.youtubeUrl, mapUrl: t.mapUrl || null,
    latitude: t.lat, longitude: t.lng,
    durationLabel: t.durationLabel, durationSeconds: t.durationSeconds,
    descriptionShort: t.descriptionShort, descriptionLong: t.descriptionLong,
    landmarks: t.landmarks, keywords: t.keywords,
    status: t.status, thumbnailPath: t.thumbnailPath || null,
    href, thumbnailSrc: t.thumbnailSrc,
  };
}
...
// Auto-generated from data/maps/germany.csv by scripts/import-germany-csv.js
// Last refreshed: ${new Date().toISOString().slice(0, 10)}
export const germanyMapFeatures: ExploreMapFeature[] = ${JSON.stringify(mapTours.map(toMapFeature), null, 2)};
`;

console.log(`  [2/2] data/maps/germany.ts — ${mapTours.length} map features`);
...
console.log(`\n  ✓ Written data/videos/germany.ts and data/maps/germany.ts`);
console.log(`${'═'.repeat(60)}\n`);
