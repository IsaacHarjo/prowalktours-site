...
const montmartreVideo = franceVideos.find(
  (video) => video.slug === "montmartre-evening-walk-2022"
);
const youtubeVideoId =
  montmartreVideo?.youtubeUrl.split("/").pop() ?? "hLa9PRc9DFk";
const heroImagePath = "/montmartre-evening-walk-2022/hero.jpg";
const fullMapUrl =
  "https://www.google.com/maps/d/viewer?mid=1mn6Ab_fs53Rwr8tgJISdZNS3xUsXkjU";
const fullMapEmbedUrl =
  "https://www.google.com/maps/d/u/0/embed?mid=1mn6Ab_fs53Rwr8tgJISdZNS3xUsXkjU";
...
const breadcrumbs = [
  ...
  { label: "Montmartre Evening Walk" },
];

const highlights = montmartreEveningWalk2022Detail.highlights;

const relatedTours = [
  {
    title: "Paris Day Walk (2020)",
    href: "/videos/paris-landmarks-day-walk-2020",
    description:
      "A 12-mile daytime route through Paris landmarks including the Panth\u00e9on, Louvre, Champs-\u00c9lys\u00e9es, and the Eiffel Tower.",
    imageSrc: "https://i.ytimg.com/vi/oz1Mgu8e1N4/maxresdefault.jpg",
    imageAlt: "Paris landmarks day walk",
  },
  {
    title: "Paris Evening Walk (2022)",
    href: "/videos/paris-evening-walk-2022",
    description:
      "A broader evening route through central Paris covering Notre-Dame, the Marais, Bastille, the Eiffel Tower, and the Louvre.",
    imageSrc: "https://i.ytimg.com/vi/fKgP6JGAM_A/maxresdefault.jpg",
    imageAlt: "Paris evening walk along the Seine at dusk",
  },
  {
    title: "Paris Latin Quarter & Marais Evening Walk (2020)",
    href: "/videos/paris-latin-quarter-marais-evening-walk-2020",
    description:
      "An evening walk through the Latin Quarter and Marais after dark, with Bastille nightlife and a night bike ride.",
    imageSrc: "/paris-latin-quarter-marais-evening-walk-2020/hero.jpg",
    imageAlt: "Paris Latin Quarter streets at night",
  },
];

export default function MontmartreEveningWalk2022Client() {
  const formattedFilmingDate = montmartreVideo?.filmingDates[0]
    ? new Date(
        `${montmartreVideo.filmingDates[0]}T12:00:00`
  ...
  const topRowStats = [
    ...
    { icon: "\ud83d\udccf", label: "Distance", value: "1.5 mi / 2.4 km" },
    {
      icon: "\ud83d\udd52",
      label: "Duration",
      value: montmartreVideo?.durationLabel ?? "55:25",
    },
  ...
  const scrollHighlights = (direction: "left" | "right") => { highlightsRef.current?.scrollBy({ left: direction === "left" ? -340 : 340, behavior: "smooth" }); };
  ...
  return (
    ...
    >
        ...
        <div className="mx-auto max-w-6xl px-6 py-12 lg:px-10 lg:py-16">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
            <div className="min-w-0">
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#9a735a]">{montmartreEveningWalk2022Detail.heroEyebrow}</p>
              <h1 className="mt-3 text-4xl font-bold tracking-tight text-[#3d3327] sm:text-5xl">{montmartreEveningWalk2022Detail.heroTitle}</h1>
              <p className="mt-3 text-xl text-[#6e5a45]">{montmartreEveningWalk2022Detail.heroSubtitle}</p>
              <p className="mt-6 max-w-3xl text-base leading-8 text-[#56493a]">{montmartreEveningWalk2022Detail.heroDescription}</p>
            ...
            <div className="relative hidden overflow-hidden rounded-[2rem] border border-[#d8c7b5] bg-white shadow-sm lg:block">
              <div className="relative aspect-[4/3] w-full">
                <img src={heroImagePath} alt="Montmartre evening walk" className="h-full w-full object-cover" loading="eager" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section ref={videoSectionRef as React.RefObject<HTMLElement>} className="mx-auto max-w-6xl px-6 pb-6 pt-6 lg:px-10 lg:pb-6 lg:pt-14">
        <div className="overflow-hidden rounded-[2rem] border border-[#d8c7b5] shadow-lg">
          <div className="aspect-video w-full bg-black">
            <iframe ref={playerIframeRef} className="h-full w-full" src={initialYoutubeEmbedUrl} title="Montmartre, France Evening Walk (2022)" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen />
          </div>
        </div>
      </section>
        ...
        <div ref={highlightsRef} className="mt-8 flex gap-4 overflow-x-auto scroll-smooth pb-2">
          {highlights.map((highlight) => (
            <div key={`${highlight.title}-${highlight.seconds}`} className="w-[280px] shrink-0">
              <button onClick={() => handleHighlightClick(highlight.seconds)} type="button" className="w-full overflow-hidden rounded-[1.5rem] border border-[#d8c7b5] bg-white text-left shadow-sm transition hover:-translate-y-1 hover:border-[#167fd5] hover:shadow-lg">
          ...
          <div className="mt-6 rounded-[1.75rem] border border-[#d8c7b5] bg-[#fcfaf6] p-5 shadow-sm">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="max-w-xl">
                ...
                <p className="mt-3 text-[15px] leading-7 text-[#56493a]">Scan to open the Montmartre evening walk route on your phone using Google Maps.</p>
              ...
              <a href={fullMapUrl} target="_blank" rel="noreferrer" className="self-start rounded-[1.5rem] border border-[#e5d7c6] bg-white p-3 shadow-sm transition hover:border-[#cdb7a0]" aria-label="Open the Montmartre route map on your phone">
                <img src={fullMapQrUrl} alt="QR code for the Montmartre evening walk route" width="132" height="132" className="h-[132px] w-[132px] rounded-xl" />
              </a>
            </div>
          </div>
        </MapSection>
        ...
        <div className="rounded-[2rem] border border-[#d8c7b5] bg-[#f7efe4] p-7 shadow-xl lg:p-8">
          <div className="grid gap-7 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-bold tracking-tight text-[#3d3327]">License this Montmartre footage</h2>
              <p className="mt-5 max-w-2xl text-base leading-8 text-[#56493a]">{montmartreEveningWalk2022Detail.licensingDescription[0]}</p>
              <p className="mt-5 max-w-2xl text-base leading-8 text-[#56493a]">{montmartreEveningWalk2022Detail.licensingDescription[1]}</p>
            </div>
                ...
                <div className="rounded-[1.25rem] border border-[#eadfce] bg-[#fcfaf6] p-4">
                  <div className="space-y-3">
                    ...
                    <div className="border-b border-[#efe3d3] pb-3">
                      ...
                      <span className="mt-1 block text-sm font-semibold text-[#5c4c33]">60 fps</span>
                    </div>
                    <div>
      ...
      <section id="related-tours" ref={relatedToursRef} className="scroll-mt-32 mx-auto max-w-6xl px-6 pb-16 lg:px-10">
        ...
        <p className="mt-4 max-w-3xl text-base leading-8 text-[#56493a]">Continue with a full-day Paris landmarks route, an evening walk through central Paris, or the Latin Quarter after dark.</p>
