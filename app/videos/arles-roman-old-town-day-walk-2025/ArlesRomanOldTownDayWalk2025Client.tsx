...
export default function ArlesRomanOldTownDayWalk2025Client() {
  const videoRecord = franceVideos.find(v => v.slug === "arles-roman-old-town-day-walk-2025");
  const youtubeVideoId = videoRecord?.youtubeUrl.split("/").pop() ?? "vRdoJsGj0Z4";
  const fullMapUrl = "https://www.google.com/maps/d/edit?mid=1GXYIJNdJctAYj8-VFd9T5ze8hv44rkc&usp=sharing";
  const fullMapEmbedUrl = "https://www.google.com/maps/d/u/0/embed?mid=1GXYIJNdJctAYj8-VFd9T5ze8hv44rkc";
  const fullMapQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(fullMapUrl)}`;
  const formattedFilmingDate = videoRecord?.filmingDates[0]
    ? new Date(`${videoRecord.filmingDates[0]}T12:00:00`).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })
    : "Wednesday, September 17, 2025";
  const topRowStats = [
    { icon: "\ud83d\udcc5", label: "Date", value: formattedFilmingDate },
    { icon: "\ud83d\udccf", label: "Distance", value: "4.5 mi / 7.2 km" },
    { icon: "\ud83d\udd52", label: "Duration", value: videoRecord?.durationLabel ?? "3:11:12" },
    { icon: "\u2600\ufe0f", label: "Weather", value: videoRecord?.weather ?? "76\u00b0F / 24\u00b0C" },
  ];
  const breadcrumbs = [
    { label: "Home", href: "/" },
  ...
  const detail = arlesRomanOldTownDayWalk2025Detail;
  const highlights = detail.highlights;
  const relatedTours = [
    { title: "Avignon Walking Tour", href: "/videos/avignon-walking-tour-2025", description: "The historic City of the Popes \u2014 Papal Palace, Pont d'Avignon, ramparts, and Les Halles.", thumbnail: "https://i.ytimg.com/vi/2iQh_R4t2Uw/maxresdefault.jpg" },
    { title: "Explore Provence", href: "/destinations/france/provence", description: "Return to the Provence hub to browse Arles, Avignon, and future regional walking tours.", thumbnail: "https://i.ytimg.com/vi/vRdoJsGj0Z4/maxresdefault.jpg" },
  ...
  return (
          ...
          <nav className="mx-auto grid max-w-6xl grid-cols-[1fr_auto] items-center px-6 py-2 sm:py-3 lg:px-10">
            <div className="justify-self-end sm:justify-self-start">
              <div className="flex min-w-max items-center gap-4 text-xs font-semibold text-white/90 sm:gap-6 sm:text-sm">
                <a href="#overview" className="hidden transition hover:text-white sm:inline-flex">Overview</a>
                <a href="#highlights" className="hidden transition hover:text-white sm:inline-flex">Highlights</a>
                <a href="#route-map" className="hidden transition hover:text-white sm:inline-flex">Route Map</a>
                <a href="#licensing" className="hidden transition hover:text-white sm:inline-flex">Licensing</a>
                <a href="#related-tours" className="hidden transition hover:text-white sm:inline-flex">Related Tours</a>
              </div>
            </div>
    ...
    >
      <section id="overview" className="scroll-mt-32 border-b border-[#d8c7b5] bg-gradient-to-br from-[#f4e6bc] via-[#fcfaf6] to-[#e7f1f8]">
      ...
      <section id="highlights" className="scroll-mt-32 mx-auto max-w-6xl rounded-[2rem] border border-[#e4d3b2] bg-gradient-to-br from-[#f4e6bc] via-[#fbf3dc] to-[#f7ede3] px-6 py-6 lg:px-10">
        <div className="flex items-end justify-between gap-4">
          ...
          <div className="hidden items-center gap-3 md:flex">
            <button onClick={() => scrollHighlights("left")} className="rounded-full border border-[#cdb7a0] bg-white px-4 py-3 text-2xl font-semibold text-[#7f5f49] shadow-sm transition hover:border-[#9a735a] hover:bg-[#fff7ee]" aria-label="Scroll highlights left">&lsaquo;</button>
            <button onClick={() => scrollHighlights("right")} className="rounded-full border border-[#cdb7a0] bg-white px-4 py-3 text-2xl font-semibold text-[#7f5f49] shadow-sm transition hover:border-[#9a735a] hover:bg-[#fff7ee]" aria-label="Scroll highlights right">&rsaquo;</button>
          </div>
        </div>
        ...
        <div ref={highlightsRef} className="mt-8 flex gap-4 overflow-x-auto scroll-smooth pb-2">
          {highlights.map((highlight) => (
            <div key={`${highlight.title}-${highlight.seconds}`} className="w-[280px] shrink-0">
              <div role="button" tabIndex={0} onClick={() => handleHighlightClick(highlight.seconds)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); handleHighlightClick(highlight.seconds); } }} className="w-full cursor-pointer overflow-hidden rounded-[1.5rem] border border-[#d8c7b5] bg-white text-left shadow-sm transition hover:-translate-y-1 hover:border-[#167fd5] hover:shadow-lg">
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <p className="text-sm font-semibold text-[#3d3327]">{highlight.title}</p>
                    <p className="shrink-0 text-sm text-[#8a7a68]">{highlight.timeLabel}</p>
                  </div>
                  <p className="mt-2 text-sm leading-6 text-[#56493a]">{highlight.description}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section id="route-map" className="scroll-mt-32 mx-auto max-w-6xl px-6 py-8 lg:px-10 lg:py-10">
        <MapSection
          eyebrow="Route map"
          heading="Explore the route"
          description={detail.routeMapDescription}
          iframeSrc={fullMapEmbedUrl}
          iframeTitle="Arles walking route map"
          ...
          <div className="mt-6 rounded-[1.75rem] border border-[#d8c7b5] bg-[#fcfaf6] p-5 shadow-sm">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="max-w-xl">
                ...
                <p className="mt-3 text-[15px] leading-7 text-[#56493a]">Scan to open the Arles walking route on your phone using Google Maps.</p>
                <a href={fullMapUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center justify-center rounded-full border border-[#167fd5] bg-white px-4 py-2 text-sm font-semibold text-[#167fd5] shadow-sm transition hover:bg-[#edf6fd]">Open the route in Google Maps</a>
              </div>
              <a href={fullMapUrl} target="_blank" rel="noreferrer" className="self-start rounded-[1.5rem] border border-[#e5d7c6] bg-white p-3 shadow-sm transition hover:border-[#cdb7a0]" aria-label="Open the Arles route map on your phone">
                <img src={fullMapQrUrl} alt="QR code for opening the Arles walking route in Google Maps" width="132" height="132" className="h-[132px] w-[132px] rounded-xl" />
              </a>
            </div>
          </div>
        </MapSection>
      ...
      <section id="licensing" className="scroll-mt-32 mx-auto max-w-6xl px-6 pb-14 lg:px-10">
        <div className="rounded-[2rem] border border-[#d8c7b5] bg-[#f7efe4] p-7 shadow-xl lg:p-8">
          <div className="grid gap-7 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-bold tracking-tight text-[#3d3327]">License this Arles footage</h2>
            ...
            <div className="rounded-[1.75rem] border border-[#d8c7b5] bg-white p-5 shadow-sm sm:p-6">
                ...
                <div className="rounded-[1.25rem] border border-[#eadfce] bg-[#fcfaf6] p-4">
                  <div className="space-y-3">
                    ...
                    <div className="border-b border-[#efe3d3] pb-3">
                      ...
                      <span className="mt-1 block text-sm font-semibold text-[#5c4c33]">Sony FE PZ 16-35mm G</span>
                    </div>
                    <div>
              ...
              <div className="mt-5 flex justify-center">
                <Link href="/licensing" className="inline-flex min-w-[220px] items-center justify-center rounded-2xl bg-[#167fd5] px-8 py-4 text-center text-base font-semibold text-white shadow-sm transition hover:bg-[#09679e]">Request License Quote</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="related-tours" className="scroll-mt-32 mx-auto max-w-6xl px-6 pb-16 lg:px-10">
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#9a735a]">Related France Tours</p>
        <h2 className="mt-3 text-3xl font-bold tracking-tight text-[#3d3327]">Explore more from Provence</h2>
        <p className="mt-4 max-w-3xl text-base leading-8 text-[#56493a]">Continue through Provence with the Avignon walking tour, or return to the France destination hub.</p>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {relatedTours.map((tour) => (
            <Link key={tour.href} href={tour.href} className="group rounded-[1.5rem] border border-[#d8c7b5] bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-[#167fd5] hover:shadow-lg">
              <div className="mb-4 overflow-hidden rounded-[1rem] border border-[#eadfce]">
                <img src={tour.thumbnail} alt={`${tour.title} thumbnail`} className="aspect-video w-full object-cover" loading="lazy" onError={(event) => { const image = event.currentTarget; image.onerror = null; image.src = tour.thumbnail.replace("/maxresdefault.jpg", "/hqdefault.jpg"); }} />
              </div>
              ...
              <p className="mt-4 text-sm font-semibold text-[#167fd5]">View tour &rarr;</p>
            </Link>
          ))}
        </div>
      </section>
        ...
        <div className="rounded-[2rem] border border-[#d8c7b5] bg-gradient-to-br from-[#f4e6bc] via-[#fcfaf6] to-[#e7f1f8] p-8 shadow-sm lg:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-bold tracking-tight text-[#3d3327]">Don&apos;t miss the next walk</h2>
              <p className="mt-4 max-w-2xl text-base leading-8 text-[#56493a]">Get updates when new walks, regional pages, and long-form France videos go live on Prowalk Tours.</p>
            ...
            <form onSubmit={(event) => { event.preventDefault(); alert("This sign-up box is just the layout for now. It still needs to be connected to an email service."); }} className="rounded-[1.75rem] border border-[#d8c7b5] bg-white p-6 shadow-sm">
              <div className="grid gap-4">
                <div>
                  <label className="mb-1.5 block text-sm font-semibold text-[#5c4c33]">First Name</label>
                  <input type="text" className="w-full rounded-xl border border-[#d8c7b5] bg-[#fcfaf6] px-4 py-2.5 text-sm outline-none transition focus:border-[#167fd5]" />
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-semibold text-[#5c4c33]">Email</label>
                  <input type="email" className="w-full rounded-xl border border-[#d8c7b5] bg-[#fcfaf6] px-4 py-2.5 text-sm outline-none transition focus:border-[#167fd5]" />
                </div>
                <button type="submit" className="mt-2 rounded-2xl bg-[#167fd5] px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#09679e]">Notify Me</button>
              </div>
            </form>
          </div>
        </div>
