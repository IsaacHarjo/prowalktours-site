...
const video = franceVideos.find((v) => v.slug === "paris-promenade-plantee-day-walk-2020");
const youtubeVideoId = video?.youtubeUrl.split("/").pop() ?? "KoORZY4TygU";
const heroImagePath = "/paris-promenade-plantee-day-walk-2020/hero.jpg";
const fullMapUrl = "https://www.google.com/maps/d/viewer?mid=13zIBRD_5PE6eg6Y8fJsrS8havyO9wc3O";
const fullMapEmbedUrl = "https://www.google.com/maps/d/u/0/embed?mid=13zIBRD_5PE6eg6Y8fJsrS8havyO9wc3O";
...
const breadcrumbs = [
  ...
  { label: "Promenade Plant\u00e9e Walk (2020)" },
];

const highlights = parisPromenadePlanteeDayWalk2020Detail.highlights;

const relatedTours = [
  { title: "Paris Day Walk (2020)", href: "/videos/paris-landmarks-day-walk-2020", description: "A 12-mile daytime route through Paris landmarks including the Panth\u00e9on, Louvre, Champs-\u00c9lys\u00e9es, and the Eiffel Tower.", imageSrc: "https://i.ytimg.com/vi/oz1Mgu8e1N4/maxresdefault.jpg", imageAlt: "Paris landmarks day walk" },
  { title: "Paris Evening Walk (2022)", href: "/videos/paris-evening-walk-2022", description: "A broader evening route through central Paris covering Notre-Dame, the Marais, Bastille, the Eiffel Tower, and the Louvre.", imageSrc: "https://i.ytimg.com/vi/fKgP6JGAM_A/maxresdefault.jpg", imageAlt: "Paris evening walk" },
  { title: "Montmartre Day Walk (2020)", href: "/videos/montmartre-day-walk-2020", description: "Moulin Rouge, Rue Lepic, La Maison Rose, Place du Tertre, and Sacr\u00e9-C\u0153ur dome views in Montmartre.", imageSrc: "https://i.ytimg.com/vi/yqOlY5uBBbo/maxresdefault.jpg", imageAlt: "Montmartre day walk" },
];

export default function ParisPromenadePlanteeDayWalk2020Client() {
  ...
  const topRowStats = [
    ...
    { icon: "\ud83d\udccf", label: "Distance", value: "3.3 mi / 5.3 km" },
    { icon: "\ud83d\udd52", label: "Duration", value: video?.durationLabel ?? "57:40" },
    { icon: "\u2600\ufe0f", label: "Weather", value: "28\u00b0C | 84\u00b0F" },
  ];
  ...
  useEffect(() => {
    ...
    const initializePlayer = () => {
      ...
      playerRef.current = new window.YT.Player(playerIframeRef.current, { events: { onReady: () => { if (pendingSeekRef.current === null) return; const s = pendingSeekRef.current; pendingSeekRef.current = null; playerRef.current?.seekTo(s, true); playerRef.current?.playVideo(); } } });
    };
    const prev = window.onYouTubeIframeAPIReady;
    window.onYouTubeIframeAPIReady = () => { prev?.(); initializePlayer(); };
  ...
  return (
    ...
    >
      <section id="overview" ref={overviewSectionRef} className="scroll-mt-32 border-b border-[#d8c7b5] bg-gradient-to-br from-[#f4e6bc] via-[#fcfaf6] to-[#e7f1f8]">
        <div className="mx-auto max-w-6xl px-6 py-12 lg:px-10 lg:py-16">
          <nav aria-label="Breadcrumb" className="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-[#8a7a68]">
            {breadcrumbs.map((item, index) => (<div key={item.label} className="inline-flex items-center gap-2">{item.href ? (<Link href={item.href} className="transition hover:text-[#167fd5]">{item.label}</Link>) : (<span className="font-medium text-[#5c4c33]">{item.label}</span>)}{index < breadcrumbs.length - 1 ? <span aria-hidden="true" className="text-[#bba893]">/</span> : null}</div>))}
          </nav>
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
            <div className="min-w-0">
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#9a735a]">{parisPromenadePlanteeDayWalk2020Detail.heroEyebrow}</p>
              <h1 className="mt-3 text-4xl font-bold tracking-tight text-[#3d3327] sm:text-5xl">{parisPromenadePlanteeDayWalk2020Detail.heroTitle}</h1>
              <p className="mt-3 text-xl text-[#6e5a45]">{parisPromenadePlanteeDayWalk2020Detail.heroSubtitle}</p>
              <p className="mt-6 max-w-3xl text-base leading-8 text-[#56493a]">{parisPromenadePlanteeDayWalk2020Detail.heroDescription}</p>
              <div className="mt-8 space-y-4 border-y border-[#d8c7b5]/80 py-4 text-[#3d3327]"><LongFormWalkStatsRow stats={topRowStats} /></div>
            </div>
            <div className="relative hidden overflow-hidden rounded-[2rem] border border-[#d8c7b5] bg-white shadow-sm lg:block"><div className="relative aspect-[4/3] w-full"><img src={heroImagePath} alt="Paris Promenade Plant\u00e9e greenway" className="h-full w-full object-cover" loading="eager" /></div></div>
          </div>
        </div>
      </section>
      ...
      <section ref={videoSectionRef as React.RefObject<HTMLElement>} className="mx-auto max-w-6xl px-6 pb-6 pt-6 lg:px-10 lg:pb-6 lg:pt-14">
        <div className="overflow-hidden rounded-[2rem] border border-[#d8c7b5] shadow-lg"><div className="aspect-video w-full bg-black">{mounted && (<iframe ref={playerIframeRef} className="h-full w-full" src={initialYoutubeEmbedUrl} title="Paris Promenade Plant\u00e9e Walk (2020)" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen />)}</div></div>
      </section>
          ...
          {highlights.map((h) => (
            <div key={`${h.title}-${h.seconds}`} className="w-[280px] shrink-0">
              <div role="button" tabIndex={0} onClick={() => handleHighlightClick(h.seconds)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); handleHighlightClick(h.seconds); } }} className="w-full cursor-pointer overflow-hidden rounded-[1.5rem] border border-[#d8c7b5] bg-white text-left shadow-sm transition hover:-translate-y-1 hover:border-[#167fd5] hover:shadow-lg">
                ...
                <div className="p-4"><div className="flex items-start justify-between gap-3"><p className="text-sm font-semibold text-[#3d3327]">{h.title}</p><p className="shrink-0 text-sm text-[#8a7a68]">{h.timeLabel}</p></div><p className="mt-2 text-sm leading-6 text-[#56493a]">{h.description}</p></div>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="mt-6 rounded-[1.75rem] border border-[#d8c7b5] bg-[#fcfaf6] p-5 shadow-sm">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="max-w-xl">
                ...
                <p className="mt-3 text-[15px] leading-7 text-[#56493a]">Scan to open the Promenade Plant\u00e9e walk route on your phone using Google Maps.</p>
              ...
              <a href={fullMapUrl} target="_blank" rel="noreferrer" className="self-start rounded-[1.5rem] border border-[#e5d7c6] bg-white p-3 shadow-sm transition hover:border-[#cdb7a0]" aria-label="Open the Promenade Plant\u00e9e route map on your phone"><img src={fullMapQrUrl} alt="QR code for the Promenade Plant\u00e9e route" width="132" height="132" className="h-[132px] w-[132px] rounded-xl" /></a>
            </div>
          </div>
        </MapSection>
      </section>
        ...
        <div className="rounded-[2rem] border border-[#d8c7b5] bg-[#f7efe4] p-7 shadow-xl lg:p-8">
          <div className="grid gap-7 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-bold tracking-tight text-[#3d3327]">License this Promenade Plant\u00e9e footage</h2>
              <p className="mt-5 max-w-2xl text-base leading-8 text-[#56493a]">{parisPromenadePlanteeDayWalk2020Detail.licensingDescription[0]}</p>
              <p className="mt-5 max-w-2xl text-base leading-8 text-[#56493a]">{parisPromenadePlanteeDayWalk2020Detail.licensingDescription[1]}</p>
            </div>
      ...
      <section id="related-tours" ref={relatedToursRef} className="scroll-mt-32 mx-auto max-w-6xl px-6 pb-16 lg:px-10">
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#9a735a]">Related Paris Tours</p>
        <h2 className="mt-3 text-3xl font-bold tracking-tight text-[#3d3327]">Explore more from Paris</h2>
        <p className="mt-4 max-w-3xl text-base leading-8 text-[#56493a]">Continue with a full Paris landmarks route, an evening walk through central Paris, or Montmartre.</p>
        ...
        <div className="rounded-[2rem] border border-[#d8c7b5] bg-gradient-to-br from-[#f4e6bc] via-[#fcfaf6] to-[#e7f1f8] p-8 shadow-sm lg:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-base leading-8 text-[#56493a]">Get updates when new Paris tours, destination pages, and long-form videos go live on ProWalk Tours.</p>
