...
const youtubeVideoId = mentonVideo?.youtubeUrl.split("/").pop() ?? "ulReotWQvO0";
...
export default function MentonFranceWalkingTour2025Page() {
  const formattedFilmingDate = mentonVideo?.filmingDates[0]
    ? new Date(`${mentonVideo.filmingDates[0]}T12:00:00`).toLocaleDateString(
        "en-US",
        {
          weekday: "long",
          month: "long",
  ...
  const topRowStats = [
    ...
    { icon: "📏", label: "Distance", value: "3.5 mi / 5.6 km" },
    {
      icon: "🕒",
      label: "Duration",
      value: mentonVideo?.durationLabel ?? "1h 47m",
    },
    {
      icon: "☀️",
      label: "Weather",
      value: mentonVideo?.weather ?? "79°F / 26°C",
    },
  ];
